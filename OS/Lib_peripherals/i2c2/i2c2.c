/*
 * SPDX-License-Identifier: MIT
 * SPDX-FileCopyrightText: 2025-2026 Edo. Franzi
 *
 * Goal:     i2c2 manager.
 */

#include    "i2c2.h"

#include    <stdint.h>
#include    <stdlib.h>

#include    "i2c_common.h"
#include    "kern/kern.h"
#include    "macros.h"
#include    "macros_core.h"
#include    "macros_soc.h"
#include    "modules.h"
#include    "os_errors.h"
#include    "record/record.h"
#include    "types.h"

#ifdef CONFIG_MAN_I2C2_S

// uKOS-X specific (see the module.h)
// ==================================

// ----------------------------------I------------I-----------------------------------------I--------------I

STRG_LOC_CONST(aStrApplication[]) = "i2c2         i2c2 manager.                             (c) EFr-2026";
STRG_LOC_CONST(aStrHelp[])        = "i2c2 manager\n"
                                    "============\n\n"

                                    "This manager ...\n\n"

                                    "Module built on "__DATE__"  "__TIME__" (c) EFr-2026\n\n";

MODULE(
    I2c2,                           // Module name (the first letter has to be upper case)
    KID_FAM_PERIPHERALS,            // Family (defined in the module.h)
    KNUM_I2C2,                      // Module identifier (defined in the module.h)
    nullptr,                        // Address of the initialisation code (early pre-init)
    nullptr,                        // Address of the code (prgm for tools, aStart for applications, nullptr for libraries)
    nullptr,                        // Address of the clean code (clean the module)
    " 1.0",                         // Revision string (major . minor)
    (1U<<BSHOW),                    // Flags (BSHOW = visible with "man", BEXE_CONSOLE = executable, BCONFIDENTIAL = hidden)
    0                               // Execution cores
);

// Library specific
// ================

static  mutx_t      *vMutex_Reserve[KNB_CORES];

// Prototypes

static  int32_t     local_init(void);

/*
 * \brief Reserve the i2c2 manager
 *
 * Call example in C:
 *
 * \code{.c}
 * int32_t    status;
 *
 *    status = i2c2_reserve(KMODE_READ_WRITE, 1234U);
 *    ....
 *    i2c2_xyz();
 *    ....
 *    status = i2c2_release(KMODE_READ_WRITE);
 * \endcode
 *
 * \param[in]   reserveMode     Any mode
 * \param[in]   timeout         Timeout (1-ms of resolution)
 *                              KWAIT_INFINITY, waiting forever
 *                              KWAIT_REMAINING_TIMEOUT, waiting for the remaining timeout
 * \return      KERR_I2C_NOERR  The manager is reserved
 * \return      KERR_I2C_GEERR  General error
 * \return      KERR_I2C_CHBSY  The manager is busy
 *
 */
int32_t i2c2_reserve([[maybe_unused]] reserveMode_t reserveMode, uint32_t timeout) {
    int32_t     status;
    uint32_t    core;

    core = GET_RUNNING_CORE;

    PRIVILEGE_ELEVATE;
    status = local_init();
    if (status != KERR_I2C_NOERR) { PRIVILEGE_RESTORE; return status; }

    status = kern_lockMutex(vMutex_Reserve[core], timeout);
    if (status != KERR_KERN_NOERR) {
        PRIVILEGE_RESTORE;
        return KERR_I2C_CHBSY;
    }

    PRIVILEGE_RESTORE;
    return KERR_I2C_NOERR;
}

/*
 * \brief Release the i2c2 manager
 *
 * Call example in C:
 *
 * \code{.c}
 * int32_t    status;
 *
 *    status = i2c2_release(KMODE_READ_WRITE);
 * \endcode
 *
 * \param[in]   reserveMode     Any mode
 * \return      KERR_I2C_NOERR  OK
 * \return      KERR_I2C_GEERR  General error
 * \return      KERR_I2C_CAREL  Cannot release the manager
 *
 */
int32_t i2c2_release([[maybe_unused]] reserveMode_t reserveMode) {
    int32_t     status;
    uint32_t    core;

    core = GET_RUNNING_CORE;

    PRIVILEGE_ELEVATE;
    status = local_init();
    if (status != KERR_I2C_NOERR) { PRIVILEGE_RESTORE; return status; }

    status = kern_unlockMutex(vMutex_Reserve[core]);
    if (status != KERR_KERN_NOERR) {
        PRIVILEGE_RESTORE;
        return KERR_I2C_CAREL;
    }

    PRIVILEGE_RESTORE;
    return KERR_I2C_NOERR;
}

/*
 * \brief Configure the i2c2 manager
 *
 * Call example in C:
 *
 * \code{.c}
 *          int32_t       status;
 * const    i2cCnf_t    configure = {
 *                            .oTimeout  = 1000u;
 *                            .oSpeed    = KI2C_100KBPS;
 *                        };
 *
 *    status = i2c2_configure(&configure);
 * \endcode
 *
 * \param[in]   *configure      Ptr on the configuration buffer
 * \return      KERR_I2C_NOERR  OK
 * \return      KERR_I2C_GEERR  General error
 *
 */
int32_t i2c2_configure(const i2cCnf_t *configure) {
    int32_t     status;

    PRIVILEGE_ELEVATE;
    status = local_init();
    if (status != KERR_I2C_NOERR) { PRIVILEGE_RESTORE; return status; }

    status = stub_i2c2_configure(configure);
    PRIVILEGE_RESTORE;
    return status;
}

/*
 * \brief Write a buffer to the i2c2 manager
 *
 * Call example in C:
 *
 * \code{.c}
 *          int32_t    status;
 * const    uint8_t    buffer[7] = { 1, 2, 3, 4, 5, 6, 7 };
 *
 *    status = i2c2_write(0x34, buffer, sizeof(buffer));
 * \endcode
 *
 * \param[in]   address         i2c device address
 * \param[in]   *buffer         Ptr on the buffer
 * \param[in]   size            Size of the buffer
 * \return      KERR_I2C_NOERR  OK
 * \return      KERR_I2C_GEERR  General error
 * \return      KERR_I2C_TIMEO  Timeout error
 *
 */
int32_t i2c2_write(uint8_t address, const uint8_t *buffer, uint16_t size) {
    int32_t     status;

    PRIVILEGE_ELEVATE;
    status = local_init();
    if (status != KERR_I2C_NOERR) { PRIVILEGE_RESTORE; return status; }

    status = stub_i2c2_write(address, buffer, size);
    PRIVILEGE_RESTORE;
    return status;
}

/*
 * \brief Read a buffer from the i2c2 manager
 *
 * Call example in C:
 *
 * \code{.c}
 * int32_t    status;
 * uint8_t    buffer[10];
 *
 * // Example 1: Read 10 bytes
 *
 *    buffer[0] = 0xFF;
 *    status = i2c2_read(0x34, buffer, 10);
 *
 * // Example 2: Write the register 56 and then read 2 bytes
 *
 *    buffer[0] = 56;
 *    status = i2c2_read(0x34, buffer, 2);
 * \endcode
 *
 * \param[in]   address         i2c device address
 * \param[out]  *buffer         Ptr on the buffer
 * \param[in]   size            Size of the buffer
 * \return      KERR_I2C_NOERR  OK
 * \return      KERR_I2C_GEERR  General error
 * \return      KERR_I2C_TIMEO  Timeout error
 *
 */
int32_t i2c2_read(uint8_t address, uint8_t *buffer, uint16_t size) {
    int32_t     status;

    PRIVILEGE_ELEVATE;
    status = local_init();
    if (status != KERR_I2C_NOERR) { PRIVILEGE_RESTORE; return status; }

    status = stub_i2c2_read(address, buffer, size);
    PRIVILEGE_RESTORE;
    return status;
}

// Local routines
// ==============

/*
 * \brief local_init
 *
 * - This function initialises the manager and
 *   has to be called at least once
 *
 */
static  int32_t local_init(void) {
            int32_t     status = KERR_I2C_NOERR;
            uint32_t    core;
    static  bool        vInit[KNB_CORES] = MCSET(false);

    core = GET_RUNNING_CORE;

    INTERRUPTION_OFF;
    if (!vInit[core]) {
        vInit[core] = true;

        if (kern_createMutex(KI2C2_MUTEX_RESERVE, &vMutex_Reserve[core]) != KERR_KERN_NOERR) { LOG(KFATAL_MANAGER, "i2c2: create mutx"); exit(EXIT_OS_PANIC); }

        status = stub_i2c2_init();
    }
    RETURN_INT_RESTORE(status);
}

#endif
