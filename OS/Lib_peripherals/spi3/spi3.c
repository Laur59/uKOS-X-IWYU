...
STRG_LOC_CONST(aStrApplication[]) = "spi3         spi3 manager.                             (c) EFr-2026";
STRG_LOC_CONST(aStrHelp[])        = "spi3 manager\n"
                                    "============\n\n"
...
MODULE(
    Spi3,                           // Module name (the first letter has to be upper case)
    KID_FAM_PERIPHERALS,            // Family (defined in the module.h)
    KNUM_SPI3,                      // Module identifier (defined in the module.h)
 ...
 *    spi3_xyz();
 *    ....
 *    status = spi3_release(KMODE_READ_WRITE);
 * \endcode
 *
 ...
 *
 */
int32_t spi3_reserve([[maybe_unused]] reserveMode_t reserveMode, uint32_t timeout) {
    int32_t     status;
    uint32_t    core;
...
/*
 * \brief Release the spi3 manager
 *
 * Call example in C:
 *
 * \code{.c}
 ...
 *
 */
int32_t spi3_release([[maybe_unused]] reserveMode_t reserveMode) {
    int32_t     status;
    uint32_t    core;
...
/*
 * \brief Configure the spi3 manager
 *
 * Call example in C:
 *
 * \code{.c}
 ...
 *                        };
 *
 *    status = spi3_configure(&configure);
 * \endcode
 *
 ...
 *
 */
int32_t spi3_configure(const spiCnf_t *configure) {
    int32_t     status;
...
/*
 * \brief Write-Read a byte to-from the spi3 manager
 *
 * Call example in C:
 *
 * \code{.c}
 ...
 *
 */
int32_t spi3_writeRead(uint8_t *data) {
    int32_t     status;
...
/*
 * \brief Generic, multiple Write-Read bytes to-from the spi3 manager
 *
 * Supported transfers
 *
 ...
 *
 */
int32_t spi3_multipleWriteRead(const uint8_t *wData, uint16_t wSize, uint8_t *rData, uint16_t rSize, uint32_t timeout) {
    int32_t     status;
...
static  int32_t local_init(void) {
    ...
    if (!vInit[core]) {
        ...
        if (kern_createMutex(KSPI3_MUTEX_RESERVE, &vMutex_Reserve[core]) != KERR_KERN_NOERR) { LOG(KFATAL_MANAGER, "spi3: create mutx"); exit(EXIT_OS_PANIC); }

        status = stub_spi3_init();
    }
