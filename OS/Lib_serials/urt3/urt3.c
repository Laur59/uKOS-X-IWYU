...
STRG_LOC_CONST(aStrApplication[]) = "urt3         urt3 manager.                             (c) EFr-2026";
STRG_LOC_CONST(aStrHelp[])        = "urt3 manager\n"
                                    "============\n\n"
...
MODULE(
    Urt3,                           // Module name (the first letter has to be upper case)
    KID_FAM_SERIALS,                // Family (defined in the module.h)
    KNUM_URT3,                      // Module identifier (defined in the module.h)
 ...
 * int32_t    status;
 *
 *    status = urt3_reserve(KMODE_WRITE, 1234U);
 *    ....
 *    urt3_xyz();
 *    ....
 ...
 *
 */
int32_t urt3_reserve(reserveMode_t reserveMode, uint32_t timeout) {
    int32_t     status;
    uint32_t    core;
 ...
 *
 */
int32_t urt3_release(reserveMode_t reserveMode) {
    int32_t     status;
    uint32_t    core;
 ...
 *
 */
int32_t urt3_configure(const urtxCnf_t *configure) {
    int32_t     status;
    ...
    status = stub_urt3_configure(configure);
    PRIVILEGE_RESTORE;
    return status;
}
...
int32_t urt3_write(const uint8_t *buffer, uint32_t size) {
    ...
    status = stub_urt3_write(buffer, size);
    PRIVILEGE_RESTORE;
    return status;
}
...
int32_t urt3_read(uint8_t *buffer, uint32_t *size) {
    ...
    status = stub_urt3_read(buffer, size);
    PRIVILEGE_RESTORE;
    return status;
}
...
int32_t urt3_getIdSemaphore(uint8_t semaphore, char_t **identifier) {
    ...
    if (semaphore == BSERIAL_SEMAPHORE_RX) { *identifier = KURT3_SEMAPHORE_RX; PRIVILEGE_RESTORE; return KERR_SERIAL_NOERR; }
    if (semaphore == BSERIAL_SEMAPHORE_TX) { *identifier = KURT3_SEMAPHORE_TX; PRIVILEGE_RESTORE; return KERR_SERIAL_NOERR; }
    PRIVILEGE_RESTORE;
...
static  int32_t local_init(void) {
    ...
    if (!vInit[core]) {
        ...
        if (kern_createMutex(KURT3_MUTEX_RESERVE_TX, &vMutex_Reserve_TX[core]) != KERR_KERN_NOERR) { LOG(KFATAL_MANAGER, "urt3: create mutx"); exit(EXIT_OS_PANIC); }

        status = stub_urt3_init();
    }
