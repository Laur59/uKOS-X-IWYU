...
STRG_LOC_CONST(aStrApplication[]) = "urt2         urt2 manager.                             (c) EFr-2026";
STRG_LOC_CONST(aStrHelp[])        = "urt2 manager\n"
                                    "============\n\n"
...
MODULE(
    Urt2,                           // Module name (the first letter has to be upper case)
    KID_FAM_SERIALS,                // Family (defined in the module.h)
    KNUM_URT2,                      // Module identifier (defined in the module.h)
 ...
 * int32_t    status;
 *
 *    status = urt2_reserve(KMODE_WRITE, 1234U);
 *    ....
 *    urt2_xyz();
 *    ....
 ...
 *
 */
int32_t urt2_reserve(reserveMode_t reserveMode, uint32_t timeout) {
    int32_t     status;
    uint32_t    core;
 ...
 *
 */
int32_t urt2_release(reserveMode_t reserveMode) {
    int32_t     status;
    uint32_t    core;
 ...
 *
 */
int32_t urt2_configure(const urtxCnf_t *configure) {
    int32_t     status;
    ...
    status = stub_urt2_configure(configure);
    PRIVILEGE_RESTORE;
    return status;
}
...
int32_t urt2_write(const uint8_t *buffer, uint32_t size) {
    ...
    status = stub_urt2_write(buffer, size);
    PRIVILEGE_RESTORE;
    return status;
}
...
int32_t urt2_read(uint8_t *buffer, uint32_t *size) {
    ...
    status = stub_urt2_read(buffer, size);
    PRIVILEGE_RESTORE;
    return status;
}
 ...
 * char_t     *identifier[2];
 *
 *    status = urt2_getIdSemaphore(BSERIAL_SEMAPHORE_RX, &identifier[0];
 *    status = urt2_getIdSemaphore(BSERIAL_SEMAPHORE_TX, &identifier[1];
 *
...
int32_t urt2_getIdSemaphore(uint8_t semaphore, char_t **identifier) {
    ...
    if (semaphore == BSERIAL_SEMAPHORE_RX) { *identifier = KURT2_SEMAPHORE_RX; PRIVILEGE_RESTORE; return KERR_SERIAL_NOERR; }
    if (semaphore == BSERIAL_SEMAPHORE_TX) { *identifier = KURT2_SEMAPHORE_TX; PRIVILEGE_RESTORE; return KERR_SERIAL_NOERR; }
    PRIVILEGE_RESTORE;
...
static  int32_t local_init(void) {
    ...
    if (!vInit[core]) {
        ...
        if (kern_createMutex(KURT2_MUTEX_RESERVE_RX, &vMutex_Reserve_RX[core]) != KERR_KERN_NOERR) { LOG(KFATAL_MANAGER, "urt2: create mutx"); exit(EXIT_OS_PANIC); }
        if (kern_createMutex(KURT2_MUTEX_RESERVE_TX, &vMutex_Reserve_TX[core]) != KERR_KERN_NOERR) { LOG(KFATAL_MANAGER, "urt2: create mutx"); exit(EXIT_OS_PANIC); }

        status = stub_urt2_init();
    }
    RETURN_INT_RESTORE(status);
}

#else
#error  "CONFIG_MAN_URT2_S SHALL be defined in project using urt2/urt2.c"
#endif
