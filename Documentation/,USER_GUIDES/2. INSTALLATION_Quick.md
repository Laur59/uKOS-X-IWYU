...
On your HD you should have the full µKOS-X package **OS_Kernel-X**. Then the following ports have to be installed:
...
Another required environment variable is **PATH_UKOS_X_PACKAGE**, path to the µKOS-X sources (usually installed from the GIT server), e.g.:

```bash
export PATH_UKOS_X_PACKAGE=${HOME}/Documents/OS_Kernel-X
```
...
PATH_UKOS_X_PACKAGE path to the µKOS-X sources
```

and updating PATH to access the toolchains.

Copy the following lines in  .bash_profile (**Ubuntu**) or .zprofile (**macOS**) or any file that you shall source before using µKOS-X.
...
### Getting tools for µKOS-X projects based on the Arm package
