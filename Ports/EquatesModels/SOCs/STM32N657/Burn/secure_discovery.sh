...
if [ -f "${PATH_UKOS}/Third_Parties/STM32/Library/n6/fsbl_discovery.noSignature" ]; then
    cp -f "${PATH_UKOS}/Third_Parties/STM32/Library/n6/fsbl_discovery.noSignature" "${SCRIPT_PATH}/fsbl_discovery.noSignature"
...
"${STM32_PROGRAMMER_CLI}" -c port=SWD mode=HOTPLUG ap=1 -el "${STM32_PROGRAMMER_BIN}/ExternalLoader/MX66UW1G45G_STM32N6570-DK.stldr" -e all
